...
public class LoginModle
{
    ...
    public CreateRoleRet CreateRole(CreateRoleReq req)
    {
        ...
        RoleTable roleTable = _db.Queryable<RoleTable>().Where(v=>v.Nickname==req.Nickname&&v.ServerId==req.GameServerId).First();
        if (roleTable != null)
        {
            //已经存在该昵称
    ...
    {
        //删除表数据
        List<RoleSkillTable>listRoleSkillTables =_db.Queryable<RoleSkillTable>().Where(v=>v.RoleID == roleid).ToList();
        ...
        foreach (var item in JobSkillMap)
        {
            ...
            if (item.Value.Type == 1||item.Value.Type == 3||item.Value.Type == 4)
            {
                ...
                roleSkillTable.Bindkey = item.Value.Type == 1?"Q":item.Value.Type==3?"F":item.Value.Type==4?"V":"";
            }
