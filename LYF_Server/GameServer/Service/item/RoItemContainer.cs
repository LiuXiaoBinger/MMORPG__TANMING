...
public class RoItemContainer
{
    ...
    // 按实例 UID 保存容器内物品；外部只能通过事务接口修改，避免绕过脏标记。
    ...
    public RoItemContainer(KnapsackType containerType, OnlineRole role)
    {
    ...
    public static RoItemContainer Create(KnapsackType type, OnlineRole role)
    {
        switch (type)
        {
    ...
    public virtual bool SwapIn(ItemBase item, int pos = InvalidItemPos)
    {
        ...
        {
            roleId = Role.mainRoleInfo.BaseInfo.RoleId;
        }
...
{
    public PlainBag(KnapsackType type, OnlineRole role) : base(type, role) { }
}

/// <summary>装备背包容器。</summary>
public class EquipBag : RoItemContainer
{
    public EquipBag(KnapsackType type, OnlineRole role) : base(type, role) { }
}

/// <summary>材料背包容器。</summary>
public class MaterialBag : RoItemContainer
{
    public MaterialBag(KnapsackType type, OnlineRole role) : base(type, role) { }
...
{
    public ConsumeBag(KnapsackType type, OnlineRole role) : base(type, role) { }
...
{
    public BodyBag(KnapsackType type, OnlineRole role) : base(type, role) { }
...
{
    public Warehouse(KnapsackType type, OnlineRole role) : base(type, role) { }
    public override bool CheckIn(ItemBase item, int pos = InvalidItemPos) { return base.CheckIn(item, pos); }
}
...
public class ShortCutBag : RoItemContainer
{
    ...
    public ShortCutBag(KnapsackType type, OnlineRole role) : 
        base(type, role) { }
...
class MoneyBag : RoItemContainer
{
    ...
    public MoneyBag(KnapsackType type, OnlineRole role) :
base(type, role) { }
